...
#diag(pco2.vcov) = dat$pco2.age.sd^2
#for(i in 1:(length(pco2)-1)){
#  for(j in (i+1):length(pco2)){
#    if(dat$pco2.loc[i] == dat$pco2.loc[j]){
#      pco2.vcov[i, j] = pco2.vcov[j, i] = 
#        0.90 * dat$pco2.age.sd[i] * dat$pco2.age.sd[j] 
#    } else{
...
dl = list(pco2.age = dat$pco2.age, lc = lc, lp = lp,
           pco2 = dat$pco2, pco2.pre = dat$pco2.pre, ts = ts, ns = nrow(dat),
           nall = nrow(dat) + length(ts))

##Parameters to save
parameters = c("pco2_m", "pco2_m.pre", "pco2_m.eps.ac", "ages", "a.r",
               "a.off")
...
p = do.call(jags.parallel, list(model.file = "code/modelCT.R", parameters.to.save = parameters, 
                                      data = dl, inits = NULL, n.chains = 4, n.iter = n.iter, 
                                      n.burnin = n.burnin, n.thin = n.thin) )
proc.time() - pt

save(p, file = "out/postCenoCTAoff.rda")
